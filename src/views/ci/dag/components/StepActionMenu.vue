      ...
      <div class="menu-body">
        ...
        <div class="plugins">
            ...
            <el-scrollbar height="220px">
              ...
              <div v-for="p in filteredPlugins" :key="p.key" class="plugin-item">
                ...
                <div class="plugin-actions">
                  <el-button size="small" type="primary" @click="choose('insert-below', p)">插入到下方</el-button>
                  <el-button size="small" @click="choose('insert-above', p)">插入到上方</el-button>
                </div>
              </div>
            </el-scrollbar>
          </div>
          <div class="more-actions">
            <el-divider />
            <el-button size="small" type="warning" @click="emitEdit">编辑当前步骤</el-button>
            <el-button size="small" type="danger" @click="emitDelete">删除当前步骤</el-button>
          </div>
        </div>
      </div>
      <div class="menu-footer">
        <el-button size="small" text @click="onCancel">关闭</el-button>
      </div>
    </div>
  </teleport>
  </template>
...
const menuStyle = computed(() => ({
  ...
  top: `${Math.max(8, Math.min(window.innerHeight - 380, props.y))}px`,
}))
...
.menu-header { display: flex; gap: 8px; align-items: center; margin-bottom: 8px; }
.menu-body { display: flex; gap: 12px; }
.categories { width: 156px; border-right: 1px solid #f0f2f5; padding-right: 8px; display: flex; flex-direction: column; gap: 6px; }
...
.plugins-title { font-size: 13px; color: #666; margin: 2px 0 6px 0; }
.plugin-list { display: flex; flex-direction: column; gap: 6px; }
.plugin-item { display: flex; justify-content: space-between; align-items: center; padding: 8px; border: 1px solid #ebeef5; border-radius: 6px; margin-bottom: 6px; }
.plugin-name { font-weight: 600; }
.plugin-desc { font-size: 12px; color: #888; }
